...
export const PriceCard = ({title, img, price, description, features}: Prop) => {
  ...
  return (
    <div className='flex flex-col items-center w-[326px] px-8 pb-14 rounded-[25px] border-coldgray border-[0.5px]'>
        ...
        <ul className="flex flex-col gap-4 w-fit">
            {
                features.map((feature, index) => (
                    <li key={index} className={clsx(worksans.className, 'flex gap-3 lg:text-base')}><CheckSVG height={24} width={24} />{feature}</li>
                ))
            }
        </ul>
    </div>
